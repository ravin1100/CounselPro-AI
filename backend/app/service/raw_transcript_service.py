...
async def create_raw_transcript(
    ...
    try:
        ...
        await db.rollback()
        logger.error(f"Database error during transcript creation: {e}")
...
async def get_raw_transcript_by_uid(
    db: AsyncSession, transcript_uid: UUID
) -> RawTranscriptResponse:
    try:
        stmt = (
            select(RawTranscript)
        ...
        logger.error(f"Database error fetching transcript {transcript_uid}: {e}")
        raise BaseAppException(
            error="Database Error",
            details=str(e),
            status_code=500,
...
async def get_raw_transcript_by_session_uid(
    ...
    try:
        ...
        logger.error(
            f"Database error fetching transcript for session {session_uid}: {e}"
        )
        raise BaseAppException(
            error="Database Error",
            details=str(e),
...
async def update_raw_transcript(
    db: AsyncSession, transcript_uid: UUID, transcript_in: RawTranscriptUpdate
) -> RawTranscriptResponse:
    try:
        stmt = (
            select(RawTranscript)
        ...
        await db.rollback()
        logger.error(f"Database error during transcript update {transcript_uid}: {e}")
        raise BaseAppException(
            error="Database Error",
            details=str(e),
            status_code=500,
...
async def delete_raw_transcript(db: AsyncSession, transcript_uid: UUID) -> dict:
    try:
        ...
        await db.rollback()
        logger.error(f"Database error during transcript deletion {transcript_uid}: {e}")
        raise BaseAppException(
            error="Database Error",
            details=str(e),
            status_code=500,
